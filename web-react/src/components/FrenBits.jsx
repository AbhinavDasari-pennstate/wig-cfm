...
// Pulsing status chip in fren headers. Shows when answers come from the real
// LLM (WIG_RUNNER=sdk on the backend) vs the deterministic scripted brain.
export function FrenLive({ className = '' }) {
  const { llmLive } = useApp();
  return (
    <div className={'fren-live ' + className} title={llmLive ? 'Answers generated by Claude' : 'Deterministic scripted answers'}>
      <span className="d" /> {llmLive ? 'Live · LLM' : 'Live'}
    </div>
  );
}
